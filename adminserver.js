...
app.post("/addnewbus", (req, res) => {
  ...
  con.query(
    "CREATE TABLE " +
      bus_id +
      " (time varchar(20), busstop(20), atime varchar(20))"
  );
  if (times.length == stops.length) {
    for (let i = 0; i < times.length; i++) {
      con.query("insert into " + bus_id + " values(?,?,?)", [
        times[i],
        stops[i],
        null,
      ]);
  ...
  const userdata = JSON.parse(
    fs.readFileSync("public/data/userbuses.json", "utf8")
  );
...
app.delete("/resetbus/:busId", (req, res) => {
  ...
  con.query("UPDATE " + busId + " SET atime = '?' WHERE 1", [null]);
...
app.delete("/totalresetbuses", (req, res) => {
  ...
  for (const key in data) {
    ...
    bus.route.forEach((element) =>
      con.query("UPDATE " + key + " SET atime = '?' WHERE 1", [null])
    );
  }
...
app.delete("/deletewaypoint/:busId/:busStop", (req, res) => {
  ...
  con.query("DELETE FROM" + busId + "WHERE busstop=" + busStop);
