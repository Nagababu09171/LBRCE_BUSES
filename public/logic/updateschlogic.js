...
async function deleteBus(busId) {
  const response = await fetch(`/admin/deletebus/${busId}`, {
    method: "DELETE",
  });
  if (!response.ok) {
    const errorMessage = await response.text();
...
async function resetBus(busId) {
  const response = await fetch(`/admin/resetbus/${busId}`, {
    method: "DELETE",
  });
  if (!response.ok) {
    const errorMessage = await response.text();
...
function addWaypoint(currentWaypoint) {
  ...
  const currentWaypointIndex = Array.from(
    currentWaypoint.parentNode.children
  ).indexOf(currentWaypoint);
  currentWaypoint.parentNode.insertBefore(
    newWaypoint,
    currentWaypoint.nextSibling
  );
...
function delWaypoint(currentWaypoint) {
  ...
  for (
    let i =
      Array.from(currentWaypoint.parentNode.children).indexOf(currentWaypoint) +
      1;
    i < numWaypoints;
    i++
  ) {
...
async function insertWaypoint(busId, time, busStop) {
  ...
  const response = await fetch(
    `/admin/insertwaypoint/${busId}/${time}/${busStop}`,
    { method: "DELETE" }
  );
  if (!response.ok) {
    const errorMessage = await response.text();
    console.error(
      `Failed to insert waypoint ${time}:${busStop} of ${busId}: ${errorMessage}`
    );
    return;
  }
  clearBusList();
...
async function deleteWaypoint(busId, busStop) {
  const response = await fetch(`/admin/deletewaypoint/${busId}/${busStop}`, {
    method: "DELETE",
  });
  if (!response.ok) {
    const errorMessage = await response.text();
    console.error(
      `Failed to delete waypoint ${busStop} of ${busId}: ${errorMessage}`
    );
    return;
  }
  clearBusList();
...
function getRoute(busId, busData) {
  ...
  for (const routeData of busData.route) {
    ...
    deleteBtn.addEventListener("click", function deleteRow() {
      ...
      const confirmation = confirm(
        `Are you sure you want to delete the bus stop "${busStop}" of bus "${busId}"?`
      );
      if (confirmation) {
  ...
  delBtn.addEventListener("click", () => {
    ...
    const confirmation = confirm(
      `Are you sure you want to delete the bus "${busId}"?`
    );
    if (confirmation) {
      deleteBus(busId);
    }
  ...
  resetBtn.addEventListener("click", () => {
    ...
    const confirmation = confirm(
      `Are you sure you want to reset the bus "${busId}"?`
    );
    if (confirmation) {
      resetBus(busId);
    }
...
function searchRoutes() {
  ...
  for (var i = 0; i < details.length; i++) {
    ...
    for (var j = 0; j < rows.length; j++) {
      if (rows[j].getAttribute("data-id").trim().toLowerCase() == source)
        passThroughSource = true;
      if (
        rows[j].getAttribute("data-id").trim().toLowerCase() == destination &&
        passThroughSource
      ) {
        passThroughDestination = true;
        passThroughBoth = true;
      } else if (
        rows[j].getAttribute("data-id").trim().toLowerCase() == destination &&
        !passThroughSource
      ) {
